...
const LiabilityManager: React.FC<LiabilityManagerProps> = ({ liabilities, onAdd, onDelete }) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgInterest = liabilities.length > 0 
      ...
      : 0;
    const highInterestCount = liabilities.filter(l => l.interestRate > 10).length;

    return { totalBalance, avgInterest, highInterestCount };
  }, [liabilities]);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <p className="text-sm font-medium text-slate-500 uppercase">High Interest Alerts</p>
          <p className={`text-2xl font-bold mt-1 ${stats.highInterestCount > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
            {stats.highInterestCount} active
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800">New Liability Record</h3>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end space-x-3">
              ...
              <button type="submit" className="px-6 py-2 bg-rose-600 text-white font-bold rounded-lg hover:bg-rose-700 shadow-sm">Save Liability</button>
            </div>
          </form>
        </div>
      )}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {liabilities.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400 italic">No liabilities recorded. Tracking your debts is essential for a healthy balance sheet.</td></tr>
              ) : (
                liabilities.map(l => (
                  <tr key={l.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      <p className="text-sm font-bold text-slate-800">{l.name}</p>
                      <p className="text-xs text-slate-500 truncate max-w-[250px]">{l.description}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">
                      <span className={l.interestRate > 10 ? 'text-amber-600 font-bold' : ''}>
                        {l.interestRate}%
                      </span>
                    </td>
